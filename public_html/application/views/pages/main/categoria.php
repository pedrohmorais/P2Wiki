...
<script>
	...
	*/
	...
	function trocaIconeSelect(icone){
		//alert(icone);
		$('#trocaGlipSpan').html('');
		$('#trocaGlipSpan').removeClass();
		$('#trocaGlipSpan').addClass('glyphicon');
		$('#trocaGlipSpan').addClass(icone);
		$('#trocaGlipSpan').attr('icone',icone);
	}
	//before .ready
	...
	function deleteCategoria(cod_categoria){
	    ...
	    $.ajax({
			...
			url : '<?php echo site_url(); ?>categoria/deleteCategoria', 
	...
	function insertCategorias(){
		...
		var glyphicon = $('#trocaGlipSpan').attr('icone');
		...
		if (user_group < 0 || user_group == '') {
		    ...
		    $.ajax({
				url : '<?php echo site_url(); ?>categoria/insertCategorias', 
				cache : false,
				async : false,
				type : 'post',
				dataType : 'json',
				...
				success: function(response,status) {
					if(response!='false'){
	                    ...
	                    linha    +=     '<td class="col-sm-1 text-center"><a class="iframe" rel="modal"  style="color: rgb(51, 51, 51);" href="<?php echo site_url(); ?>categoria/editar_categoria/'+ response + '"><span class=" hidden-xs showopacity glyphicon glyphicon-pencil" style="font-size:16px;"></span></a></td>';

	                    linha    +=     '<td class="col-sm-1 text-center"><a class="iframe" rel="modal"  style="color: rgb(51, 51, 51);" href="<?php echo site_url(); ?>categoria/topicos/'+ response + '"><span class=" hidden-xs showopacity glyphicon glyphicon-th-list" style="font-size:16px;"></span></a></td>';
	    ...
	    <div class="container">
	      ...
	      <div class="row row-offcanvas row-offcanvas-left">
	        ...
	        <ul id="ulTabs" class="nav nav-pills">
			    ...
			    <li role="presentation" class="active"><a href="<?php echo site_url(); ?>categoria">Categorias</a></li>
	                ...
	                <thead>
	                    ...
	                    <tr class="filters">		
		                    ...
		                    <th class="col-sm-3">
		                    	<select id='categoria_pai' class="form-control">
			                    	...
			                    	foreach ($categorias_base as $cat) {
			                    		echo 	'<option value="'.$cat['cod_categoria'].'">'.$cat['cod_categoria'].' - '.$cat['nome_categoria'].'</option>';		
			                    	}
			                    	?>
			                    </select>
		                    </th>
		                    ...
		                    <th class="col-sm-1" style="text-align:center;">
		                    	<!--<span class="" for="" onclick="" style="display: inline-block; margin-right: -5px; border: 1px solid rgb(204, 204, 204); border-radius: 5px 0px 0px 5px; padding: 6px 20px;">Ícone</span>
		                    	<select class="form-control" id="glyphicon" style="display: inline-block;padding: 0px; width: 18px;" onchange="trocaIconeSelect();"> -->
		                    		<!-- <option value='' selected>Ícone</option> -->
		                    		<div class="btn-group">
		                    			<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="color: rgb(85, 85, 85); font-weight: 600;"><span id="trocaGlipSpan">Ícone</span> <span class="caret"></span></button>
		                    			<ul class="dropdown-menu" style="text-align: center; width: 410px;">
		                ...
		                <?php
		                	if(count($categorias_base > 0)){
			                	foreach($categorias_base as $key=>$value)
								{
				                   ...
				                   echo    '<td class="col-sm-1 text-center"><a class="iframe" rel="modal"  style="color: rgb(51, 51, 51);" href="'.site_url('categoria/editar_categoria').'/'.$categorias_base[$key]['cod_categoria'].'"><span class=" hidden-xs showopacity glyphicon glyphicon-pencil" style="font-size:16px;"></span></a></td>';

				                   echo    '<td class="col-sm-1 text-center"><a class="iframe" rel="modal"  style="color: rgb(51, 51, 51);" href="'.site_url('categoria/topicos').'/'.$categorias_base[$key]['cod_categoria'].'"><span class=" hidden-xs showopacity glyphicon glyphicon-th-list" style="font-size:16px;"></span></a></td>';
